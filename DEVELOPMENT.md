...
- **Linear hypothesis class.** `h_θ` cannot express interactions between
  ...
  try.
- **The feature set is small and hand-designed.** Six features, of which two
  are already close to standard heuristics. Discovering features rather than
  weights is a separate and harder problem.
...
1. **A second domain.** Gripper or Logistics, added as a generator alongside
...
2. **Held-out evaluation as the default protocol**, with several optimiser
   seeds per configuration and the variance reported. The present numbers are
   single-seed point estimates.
3. **Evolutionary search over the weight vector** (CMA-ES or a simple
...
4. **Feature discovery rather than weight fitting**, via a small grammar over
   ...
   something.
5. **Reinforcement learning.** Two distinct formulations are worth separating:
...
6. **Epistemic planning.** The motivating setting. It requires a domain type
